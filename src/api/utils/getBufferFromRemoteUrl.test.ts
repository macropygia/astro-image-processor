...
describe('Unit/api/utils/getBufferFromRemoteUrl', () => {
  ...
  test.concurrent('404', async () => {
    ...
    ).rejects.toThrowError('Failed to download: http://localhost:4320/status/404 (404)');
  });

  test.concurrent('timeout', async () => {
    await expect(getBufferFromRemoteUrl('http://localhost:4320/throw', 100)).rejects.toThrowError(
      'Fetch timeout',
    );
  });
});
