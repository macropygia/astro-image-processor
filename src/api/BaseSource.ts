...
import { resolveSourceFilePath } from './utils/resolveSourceFilePath.js';
...
export class BaseSource {
  ...
  options: Omit<ImgProcProcessorOptions, 'width' | 'height' | 'formatOptions'> & {
    src: string;
  ...
  protected constructor({ ctx, componentType, options }: BaseSourceArgs) {
    ...
    if (HTTPS_RE.test(src)) {
      ...
      this.type = 'local';
      this.localSourcePath = normalizePath(resolveSourceFilePath({ dirs: this.dirs, src }));
    } else {
  ...
  public get profile(): string | Record<string, unknown> | Record<string, unknown>[] | undefined {
  ...
  protected async download(): Promise<Buffer> {
    ...
    const { buffer, expiresAt: rawExpiresAt } = await getBufferFromRemoteUrl(src, timeoutDuration);
